		...
		<CRow>
			<CCol md="8" sm="12">
				<div class="form-group">
					<label>Nomor BA Penolakan SBP</label>
					<v-autocomplete
						v-model="value"
						outlined
						dense
					...
					>
						<template v-slot:no-data>
							<v-list-item>
								<v-list-item-title>
									Data BA Penolakan SBP tidak ditemukan
								</v-list-item-title>
							</v-list-item>
						</template>
			...
			<CCol md="3" sm="12">
				<CInput
					label="Tanggal BA Penolakan SBP"
...
export default {
	...
	methods: {
		...
		async getData(id, mounted=false) {
			if (id != null) {
				let responses = await api.getDisplayDataById('tolak1', id)
				this.data = {
					id: responses.data.data.id,
					no_dok_lengkap: responses.data.data.no_dok_lengkap,
					tanggal_dokumen: responses.data.data.tanggal_dokumen
				}
				this.exception = this.data.id
				if (mounted == true) {
	...
	watch: {
		async search (val) {
			...
			let responses = await api.searchDoc('tolak1', data)
