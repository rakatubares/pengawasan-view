...
import DefaultBukaPengaman from './DefaultBukaPengaman'
...
export default {
	...
	data() {
		return {
			...
			document: JSON.parse(JSON.stringify(DefaultBukaPengaman.data)),
		}
	},
	watch: {
		state(val) {
	...
	methods: {
		...
		fillNull() {
			if (this.document.sprint == null) {
				this.document.sprint = JSON.parse(JSON.stringify(DefaultBukaPengaman.data.sprint))
			}

			if (this.document.asal_pengaman == null) {
				this.document.asal_pengaman = DefaultBukaPengaman.data.asal_pengaman
			}

			if (this.document.saksi == null) {
				this.document.saksi = JSON.parse(JSON.stringify(DefaultBukaPengaman.data.saksi))
			}
			...
			if (
				...
				this.document.petugas.petugas2 = JSON.parse(JSON.stringify(DefaultBukaPengaman.data.petugas.petugas2))
			}
		},
		closeModal() {
	...
	async beforeMount() {
		if (['show', 'edit'].includes(this.state)) {
			...
			this.document.tanggal_buka_pengaman = converters.currentDate()
		}
	},
}
</script>
