<template>
	<div class="wrapper" data-app>
		<!-- Display table data for BA Buka Tanda Pengaman list -->
		<CRow>
			<CCol>
				<MyTableData
					state="list"
				...
				>
					<template #header>
						<CIcon name="cil-grid"/>Daftar BA Pembukaan Tanda Pengaman
		...
		<!-- Modal input BA Buka tanda pengaman -->
		<MyModalBukaPengaman
...
export default {
	...
	methods: {
		async getDataTable() {
			this.list_table = await api2.getListDocuments('bukapengaman')
		},
		saveData(state) {
			if (state == 'insert') {
				this.displayTab(1)
