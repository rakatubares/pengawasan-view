...
export default {
	...
	methods: {
		publishDoc() {
			axios
				.put(api.publishPenitipan(this.id))
				.then(
					(response) => { 
						this.alert('BA Penitipan berhasil diterbitkan', 'success', 2)
						this.showPdf()
		...
		async getData() {
			const response =  await axios.get(api.getPenitipanComplete(this.id))
		...
		createPDF() {
			...
			////// NOMOR SURAT //////
			let doc_title = 'BERITA ACARA PENITIPAN'
			...
			ln += pdf_props.font.height
			doc.text('Lokasi Penitipan', pdf_props.ind.alp, ln)
			doc.text(':', pdf_props.ind.cln2, ln)
			doc.text(converters.string(this.data.lokasi_titip).replace('\n', ' '), pdf_props.ind.txt2, ln)
			ln += pdf_props.font.height
			...
			doc.text(arr_penerima, pdf_props.ind.alp, ln)
			ln += pdf_props.font.height*arr_penerima.length

			doc.text('Nama', pdf_props.ind.alp, ln)
			doc.text(':', pdf_props.ind.cln2, ln)
			doc.text(converters.string(this.data.penerima.nama), pdf_props.ind.txt2, ln)
			ln += pdf_props.font.height
			doc.text('Alamat', pdf_props.ind.alp, ln)
			doc.text(':', pdf_props.ind.cln2, ln)
			doc.text(converters.string(this.data.penerima.alamat).replace('\n', ' '), pdf_props.ind.txt2, ln)
			ln += pdf_props.font.height
			doc.text('Jabatan', pdf_props.ind.alp, ln)
			doc.text(':', pdf_props.ind.cln2, ln)
			doc.text(converters.string(this.data.penerima.pekerjaan), pdf_props.ind.txt2, ln)
			ln += pdf_props.font.height

			let txt_segel = 'Sesuai dengan BA Penyegelan Nomor : '
				+ this.data.nomor_ba_segel
				+ ' tanggal '
			...
			////// TTD //////
			let ln_tgl = ln + pdf_props.font.height*3
			...
			// Pemilik/kuasa
			doc.text('Orang yang menerima penitipan,', pdf_props.ind.alp, ln_jabatan_1)
			doc.text(this.data.penerima.nama, pdf_props.ind.alp, ln_nama_1)
			doc.text('Pemilik/kuasanya', pdf_props.ind.alp, ln_nip_1)

			// Saksi
			doc.text(this.data.saksi.nama, pdf_props.ind.alp, ln_nama_2)
			doc.text('Saksi', pdf_props.ind.alp, ln_nip_2)

			// Pejabat
			doc.text('Tangerang, ' + full_tgl_dok, pdf_props.ind.ttd, ln_tgl)
			doc.text('Pejabat yang melakukan penitipan,', pdf_props.ind.ttd, ln_jabatan_1)
			...
			////// KETERANGAN //////
			let ln_coret = ln_nip_2 + pdf_props.font.height*2
