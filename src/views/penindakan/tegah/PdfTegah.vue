...
export default {
	...
	methods: {
		publishDoc() {
			axios
				.put(api.publishPenegahan(this.id))
				.then(
					(response) => { 
						this.alert('BA Penegahan berhasil diterbitkan', 'success', 2)
						this.showPdf()
		...
		async getData() {
			const response =  await axios.get(api.getPenegahanComplete(this.id))
		...
		createPDF() {
			...
			////// NOMOR SURAT //////
			let doc_title = 'BERITA ACARA PENEGAHAN'
			...
			doc.text(arr_pernyataan, pdf_props.ind.alp, ln)
			ln += pdf_props.font.height*arr_pernyataan.length
			...
			// Sarkut
			let res_sarkut = pdf.detail_sarkut(doc, this.data.detail.sarkut, ln, pdf_props.font.height, indents, true)
			doc = res_sarkut[0]
			ln = res_sarkut[1]
			...
			let txt_saksi = 'Penegahan disaksikan oleh pengangkut/pemilik/importir/eksportir '
			...
			ln += pdf_props.font.height
			doc.text('Nomor Identitas', pdf_props.ind.alp, ln)
			doc.text(':', pdf_props.ind.cln2, ln)
			...
			// Pemilik/kuasa
			doc.text('Pemilik/Importir/Eksportir/Kuasanya/Saksi*', pdf_props.ind.alp, ln_jabatan_1)
			doc.text(this.data.saksi.nama, pdf_props.ind.alp, ln_nama_1)

			// Pejabat
			doc.text('Tangerang, ' + full_tgl_dok, pdf_props.ind.ttd, ln_tgl)
			doc.text('Pejabat yang melakukan penegahan,', pdf_props.ind.ttd, ln_jabatan_1)
			...
			////// KETERANGAN //////
			let ln_ket = ln_nip_2 + pdf_props.font.height*3

			doc.setFont('Helvetica', 'italic')
			let ket = 'Penegahan merupakan kewenangan administratif berdasarkan '
			ket += 'Pasal 77 Undang-Undang nomor 10 tahun 1995 sebagaimana diubah terakhir dengan '
			ket += 'Undang-Undang nomor 17 tahun 2006 tentang Kepabeanan dan '
			ket += 'Pasal 33 Undang-Undang nomor 11 tahun 1995 sebagaimana diubah terakhir dengan '
			ket += 'Undang-Undang nomor 39 tahun 2007 tentang Cukai.'
			let arr_ket = converters.array_text(ket, 105)
			doc.text(arr_ket, pdf_props.ind.alp, ln_ket)
			let ln_coret = ln_ket + pdf_props.font.height*(arr_ket.length + 1)
