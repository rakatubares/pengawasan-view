...
import DefaultRiksa from './DefaultRiksa'
...
export default {
	...
	data() {
		return {
			...
			document: JSON.parse(JSON.stringify(DefaultRiksa.data)),
		}
	},
	watch: {
		state(val) {
	...
	methods: {
		...
		fillNull() {
			if (this.document.penindakan.sprint == null) {
				this.document.penindakan.sprint = JSON.parse(JSON.stringify(DefaultRiksa.data.penindakan.sprint))
			}

			if (this.document.penindakan.saksi == null) {
				this.document.penindakan.saksi = JSON.parse(JSON.stringify(DefaultRiksa.data.penindakan.saksi))
			}
			...
			if (
				...
				this.document.penindakan.petugas.petugas2 = JSON.parse(JSON.stringify(DefaultRiksa.data.penindakan.petugas.petugas2))
			}
		},
		closeModal() {
