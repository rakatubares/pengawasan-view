<template>
	<div class="wrapper">
						...
						>
							<template v-slot:append-outer>
								...
								<CButton 
									...
									@click="insertEntitas"	
								>
									<CIcon class="m-0" name="cil-building"/>
								</CButton>
							</template>
		...
		<MyModalEntitasBadanHukum
			ref="modal_entitas_badan_hukum"
...
import MyModalEntitasBadanHukum from './ModalEntitasBadanHukum.vue'
	...
	methods: {
		async searchData(val) {
			if (val != null) {
				if (val != '') {
					let data = {'search': val}
					let response = await api.searchEntitasBadanHukum(data)
		...
		insertEntitas() {
			this.$refs.modal_entitas_badan_hukum.showModal('insert')
		},
		updateEntitas() {
			this.$refs.modal_entitas_badan_hukum.showModal('update', this.entitas.id)
		},
