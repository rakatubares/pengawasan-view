	...
	<div class="wrapper">
		...
		<CRow>
			<CCol md="8" sm="12">
				<CInput
					label="No LP"
					:value.sync="doc.dokumen.lp.no_dok_lengkap"
					disabled
				/>
			</CCol>
			<CCol md="4" sm="12">
				<CInput
					label="Tanggal"
					:value.sync="doc.dokumen.lp.tanggal_dokumen"
					disabled
				/>
			</CCol>
		</CRow>
...
export default {
	...
	methods: {
		...
		async getData(id, mounted=false) {
			if (id != null) {
				let responses = await api.getDisplayDataById('split', id)
	...
	watch: {
		async search (val) {
			...
			let responses = await api.searchDoc('split', data)
