		...
		<CForm class="pt-3">
				...
				<CRow>
					...
					<CCol md="2" sm="12">
						<CInput
							...
							:value.sync="data.detail.data.no_flight_trayek"
						/>
					</CCol>
				</CRow>
				<CRow>
			...
			<CRow class="sep">
				<CCol md="8">
					...
					<CRow>
						<CCol>
							<div class="input-container" v-for="(val,index) in data.tembusan" :key="index">
								<MyComboboxTembusan
									:value.sync="data.tembusan[index]"
...
export default {
	...
	methods: {
		...
		async saveData() {
			...
			// Fill null
			if (['entitas'].includes(this.data.detail.data)) {
