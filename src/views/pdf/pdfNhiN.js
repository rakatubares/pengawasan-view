...
class PdfNhiN extends Pdf {
	...
	generatePdf() {
		...
		this.pdf.text(this.data.nomor_lkain, this.props.ind.val2, this.ln)
		this.ln += this.props.font.height
		...
		this.pdf.text(':', this.props.ind.cln2, this.ln)
		this.pdf.text(converters.fullDate(this.data.tanggal_lkain), this.props.ind.val2, this.ln)
		this.ln += this.props.font.height
		...
		let arr_kantor = ''
		let hgt_kantor = 1
		if (this.data.kantor_bc) {
			arr_kantor = converters.array_text(this.data.kantor_bc.nama_kantor, 43)
		...
		if (this.data.flag_exim) {
			...
			var arr_sarkut_exim = converters.array_text(txt_sarkut_exim, 55)
		...
		let height_barang = 1
		let uraian_barang = ''
		if (this.data.barang_exim) {
			if (this.data.barang_exim.data.item.length > 1) {
				uraian_barang = 'LIHAT LAMPIRAN'
			} else {
				let txt_barang = `${this.data.barang_exim.data.item[0].jumlah_barang} `
					+ `${this.data.barang_exim.data.item[0].satuan.kode_satuan} `
					+ `${this.data.barang_exim.data.item[0].uraian_barang} `
				uraian_barang = converters.array_text(txt_barang, 50)
				height_barang = uraian_barang.length
			}	
		}
		this.pdf.text(uraian_barang, this.props.ind.val3, this.ln, {align: 'justify', maxWidth: 90})
		...
		let arr_lain_exim = converters.array_text(txt_lain_exim, 55)
		let hgt_lain_exim = arr_lain_exim.length > 1 ? arr_lain_exim.length : 1
		this.pdf.text(txt_lain_exim, this.props.ind.val3, this.ln, {align: 'justify', maxWidth: 95})
		...
		this.pdf.text(':', this.props.ind.cln3, this.ln)
		let txt_nama_sarkut = this.data.flag_sarkut ? (this.data.nama_sarkut || '') : ''
		...
		this.pdf.text(':', this.props.ind.cln3, this.ln)
		let txt_jenis_sarkut = this.data.flag_sarkut ? (this.data.jenis_sarkut || '') : ''
		...
		this.pdf.text(':', this.props.ind.cln3, this.ln)
		let txt_flight_trayek = this.data.flag_sarkut ? (this.data.no_flight_trayek_sarkut || '') : ''
		...
		this.pdf.text(':', this.props.ind.cln3, this.ln)
		let txt_asal_sarkut = ''
		if (this.data.flag_sarkut) {
			if (this.data.pelabuhan_asal_sarkut) {
				if (this.data.pelabuhan_asal_sarkut.negara) {
					txt_asal_sarkut = `${this.data.pelabuhan_asal_sarkut.airport_name}, ${this.data.pelabuhan_asal_sarkut.negara.kode_2}`
				} else {
					txt_asal_sarkut = this.data.pelabuhan_asal_sarkut.airport_name
				}
			}
		}
		let arr_asal_sarkut = converters.array_text(txt_asal_sarkut, 60)
		let hgt_asal_sarkut = arr_asal_sarkut.length > 1 ? arr_asal_sarkut.length : 1
		this.pdf.text(arr_asal_sarkut, this.props.ind.val3, this.ln)
		this.ln += this.props.font.height*hgt_asal_sarkut
		...
		if (this.data.flag_sarkut) {
			if (this.data.pelabuhan_tujuan_sarkut) {
				if (this.data.pelabuhan_tujuan_sarkut.negara) {
					txt_tujuan_sarkut = `${this.data.pelabuhan_tujuan_sarkut.airport_name}, ${this.data.pelabuhan_tujuan_sarkut.negara.kode_2}`
				} else {
					txt_tujuan_sarkut = this.data.pelabuhan_tujuan_sarkut.airport_name
				}
			}
		}
		let arr_tujuan_sarkut = converters.array_text(txt_tujuan_sarkut, 60)
		let hgt_tujuan_sarkut = arr_tujuan_sarkut.length > 1 ? arr_tujuan_sarkut.length : 1
		this.pdf.text(arr_tujuan_sarkut, this.props.ind.val3, this.ln)
		this.ln += this.props.font.height*hgt_tujuan_sarkut

		this.pdf.text('6.', this.props.ind.lbl3, this.ln)
		this.pdf.text('IMO / MMSI', this.props.ind.lbl4, this.ln)
		this.pdf.text(':', this.props.ind.cln3, this.ln)
		let txt_imo_mmsi = this.data.flag_sarkut ? (this.data.imo_mmsi_sarkut || '') : ''
		...
		this.pdf.text('Data Lainnya', this.props.ind.lbl4, this.ln)
		this.pdf.text(':', this.props.ind.cln3, this.ln)
		let txt_lain_sarkut = this.data.flag_sarkut ? (this.data.data_lain_sarkut || '') : ''
		let arr_lain_sarkut = converters.array_text(txt_lain_sarkut, 55)
		let hgt_lain_sarkut = arr_lain_sarkut.length > 1 ? arr_lain_sarkut.length : 1
		this.pdf.text(txt_lain_sarkut, this.props.ind.val3, this.ln, {align: 'justify', maxWidth: 95})
		...
		this.pdf.text('Nama', this.props.ind.lbl4, this.ln)
		this.pdf.text(':', this.props.ind.cln3, this.ln)
		let txt_nama_orang = this.data.flag_orang ? (this.data.orang.nama || '') : ''
		...
		if (this.data.flag_orang) {
			txt_jenis_kelamin = this.data.orang.jenis_kelamin.uraian
		}
		...
		this.pdf.text(':', this.props.ind.cln3, this.ln)
		let txt_warga_negara = this.data.flag_orang 
			? this.data.orang.warga_negara 
				? this.data.orang.warga_negara.nama_negara
				: ''
			: ''
		...
		this.pdf.text(':', this.props.ind.cln3, this.ln)
		let txt_no_paspor = this.data.flag_orang ? (this.data.orang.nomor_identitas || '') : ''
		...
		this.pdf.text(':', this.props.ind.cln3, this.ln)
		let txt_flight_voyage_orang = this.data.flag_orang ? (this.data.flight_voyage_orang || '') : ''
		...
		let txt_asal_orang = ''
		if (this.data.flag_orang) {
			if (this.data.pelabuhan_asal_orang) {
				if (this.data.pelabuhan_asal_orang.negara) {
					txt_asal_orang = `${this.data.pelabuhan_asal_orang.airport_name}, ${this.data.pelabuhan_asal_orang.negara.kode_2}`
				} else {
					txt_asal_orang = this.data.pelabuhan_asal_orang.airport_name
				}
			}
		}
		let arr_asal_orang = converters.array_text(txt_asal_orang, 60)
		let hgt_asal_orang = arr_asal_orang.length > 1 ? arr_asal_orang.length : 1
		this.pdf.text(arr_asal_orang, this.props.ind.val3, this.ln)
		this.ln += this.props.font.height*hgt_asal_orang
		...
		if (this.data.flag_orang) {
			if (this.data.pelabuhan_tujuan_orang) {
				if (this.data.pelabuhan_tujuan_orang.negara) {
					txt_tujuan_orang = `${this.data.pelabuhan_tujuan_orang.airport_name}, ${this.data.pelabuhan_tujuan_orang.negara.kode_2}`
				} else {
					txt_tujuan_orang = this.data.pelabuhan_tujuan_orang.airport_name
				}
			}
		}
		let arr_tujuan_orang = converters.array_text(txt_tujuan_orang, 60)
		let hgt_tujuan_orang = arr_tujuan_orang.length > 1 ? arr_tujuan_orang.length : 1
		this.pdf.text(arr_tujuan_orang, this.props.ind.val3, this.ln)
		this.ln += this.props.font.height*hgt_tujuan_orang

		this.pdf.text('8.', this.props.ind.lbl3, this.ln)
		this.pdf.text('Waktu Keberangkatan', this.props.ind.lbl4, this.ln)
		this.pdf.text(':', this.props.ind.cln3, this.ln)
		let txt_waktu_berangkat_orang = this.data.flag_orang ? (this.data.waktu_berangkat_orang || '') : ''
		...
		this.pdf.text(':', this.props.ind.cln3, this.ln)
		let txt_waktu_datang_orang = this.data.flag_orang ? (this.data.waktu_datang_orang || '') : ''
		...
		this.pdf.text('Data Lainnya', this.props.ind.lbl4, this.ln)
		this.pdf.text(':', this.props.ind.cln3, this.ln)
		let txt_lain_orang = this.data.flag_orang ? (this.data.data_lain_orang || '') : ''
		let arr_lain_orang = converters.array_text(txt_lain_orang, 55)
		let hgt_lain_orang = arr_lain_orang.length > 1 ? arr_lain_orang.length : 1
		this.pdf.text(txt_lain_orang, this.props.ind.val3, this.ln, {align: 'justify', maxWidth: 95})
		...
		if (this.data.barang_exim) {
			if (this.data.barang_exim.data.item.length > 1) {
				...
				// Tabel barang
				this.tabelBarang(this.data.barang_exim.data.item)
			}	
		}
