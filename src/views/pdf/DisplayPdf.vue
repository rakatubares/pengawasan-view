...
export default {
        ...
        async getPdf(doc_type, doc_id) {
            ...
            switch (doc_type) {
                ...
                case 'nhi':
                    ...
                    let barang_nhi = response.data
                    pdf = new PdfNhi(data_pdf, barang_nhi)
                    break;

                case 'nhin':
                    let barang_nhin = null
                    if (data_pdf.detail.type == 'nhin-exim') {
                        let response = await api.getBarang(doc_type, doc_id)
                        barang_nhin = response.data
                    }
                    pdf = new PdfNhiN(data_pdf, barang_nhin)
                    break;
    ...
    mounted() {
        if (this.show_button) { this.listPdf() }
