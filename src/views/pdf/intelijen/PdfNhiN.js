...
class PdfNhiN extends Pdf
{
    ...
    generateText() 
    {
        ...
        if (this.data.waktu_indikasi) {
            waktu = ` / ${this.data.waktu_indikasi}`
            if (this.data.zona_waktu) {
                waktu += ` ${this.data.zona_waktu}`
            }
        }
        ...
        this.txt.dok_exim = detail_type == 'nhin-exim'
            ? this.converters.no_tanggal_dok(detail_data.nomor_dok, detail_data.tanggal_dok, detail_data.jenis_dok)
            : ''
        ...
        this.txt.awb_exim = detail_type == 'nhin-exim'
            ? this.converters.no_tanggal_dok(detail_data.nomor_awb, detail_data.tanggal_awb)
            : ''
        ...
        if (detail_type == 'nhin-exim') {
            if (detail_data.entitas != null) {
                entitas_exim = this.converters.string(detail_data.entitas.data.nama)
            }
        }
        this.txt.entitas_exim = entitas_exim
        let txt_npwp_exim = ''
        if (detail_type == 'nhin-exim') {
            if (detail_data.entitas != null) {
        ...
        this.txt.nomor_sarkut = detail_type == 'nhin-sarkut'
            ...
            : ''
        let txt_asal_sarkut = ''
        if (detail_type == 'nhin-sarkut') {
            if (detail_data.pelabuhan_asal) {
                txt_asal_sarkut = detail_data.pelabuhan_asal.airport_name
                if (detail_data.pelabuhan_asal.negara) {
                    txt_asal_sarkut += `, ${detail_data.pelabuhan_asal.negara.kode_2}`
                }
            }
        }
        this.txt.asal_sarkut = txt_asal_sarkut
        let txt_tujuan_sarkut = ''
        if (detail_type == 'nhin-sarkut') {
            if (detail_data.pelabuhan_tujuan) {
                txt_tujuan_sarkut = detail_data.pelabuhan_tujuan.airport_name
                if (detail_data.pelabuhan_tujuan.negara) {
                    txt_tujuan_sarkut += `, ${detail_data.pelabuhan_tujuan.negara.kode_2}`
                }
            }
        }
        this.txt.tujuan_sarkut = txt_tujuan_sarkut
        ...
        let txt_kewarganegaraan = ''
        let txt_paspor = ''
        let txt_nomor_sarkut_orang = ''
        let txt_asal_orang = ''
        let txt_tujuan_orang = ''
        let txt_waktu_berangkat = ''
        let txt_waktu_datang = ''
        let txt_data_lain_orang = ''

        if (detail_type == 'nhin-orang') {
            if (detail_data.entitas) {
                txt_nama_orang = detail_data.entitas.nama
                
                if (detail_data.entitas.jenis_kelamin) {
                    txt_jenis_kelamin = detail_data.entitas.jenis_kelamin.uraian
                }
                
                if (detail_data.entitas.warga_negara) {
                    txt_kewarganegaraan = detail_data.entitas.warga_negara.nama_negara
                }
            ...
            txt_nomor_sarkut_orang = this.converters.string(detail_data.nomor_sarkut)

            if (detail_data.pelabuhan_asal) {
                txt_asal_orang = detail_data.pelabuhan_asal.airport_name
                if (detail_data.pelabuhan_asal.negara) {
                    txt_asal_orang += `, ${detail_data.pelabuhan_asal.negara.kode_2}`
                }
            }

            if (detail_data.pelabuhan_tujuan) {
                txt_tujuan_orang = detail_data.pelabuhan_tujuan.airport_name
                if (detail_data.pelabuhan_tujuan.negara) {
                    txt_tujuan_orang += `, ${detail_data.pelabuhan_tujuan.negara.kode_2}`
                }
            }

            if (detail_data.tanggal_berangkat) {
                txt_waktu_berangkat = detail_data.tanggal_berangkat
                if (detail_data.waktu_berangkat) {
                    txt_waktu_berangkat += ` ${detail_data.waktu_berangkat}`
                }
            }

            if (detail_data.tanggal_datang) {
                txt_waktu_datang = detail_data.tanggal_datang
                if (detail_data.waktu_datang) {
                    txt_waktu_datang += ` ${detail_data.waktu_datang}`
                }
            }

            txt_data_lain_orang = this.converters.string(detail_data.data_lain)
        }
        
        this.txt.nama_orang = txt_nama_orang
        this.txt.jenis_kelamin = txt_jenis_kelamin
        this.txt.kewarganegaraan = txt_kewarganegaraan
        this.txt.paspor = txt_paspor
        this.txt.nomor_sarkut_orang = txt_nomor_sarkut_orang
        this.txt.asal_orang = txt_asal_orang
        this.txt.tujuan_orang = txt_tujuan_orang
        this.txt.waktu_berangkat = txt_waktu_berangkat
        this.txt.waktu_datang = txt_waktu_datang
        this.txt.data_lain_orang = txt_data_lain_orang
